...
impl Parser {
    ...
    fn peek_n(&self, n: usize) -> &Token {
        self.tokens
            .get(self.pos + n)
            .unwrap_or_else(|| self.tokens.last().expect("parser token stream is empty"))
    }
    ...
    fn at_ident(&self, expected: &str) -> bool {
        self.peek().kind.is_ident(expected)
    }

    fn eat_ident(&mut self, expected: &str) -> Result<Span> {
        let token = self.peek().clone();

        if !token.kind.is_ident(expected) {
            return Err(Error::parse(format!("expected '{}'", expected), token.span));
        }
    ...
    fn eat_ident_name(&mut self) -> Result<(String, Span)> {
        ...
        match token.kind {
            ...
            _ => Err(Error::parse("expected identifier", token.span)),
        }
    }
    ...
    fn parse_assignment(&mut self) -> Result<Expr> {
        ...
        if let Some(op) = assignment_binop(&self.peek().kind) {
            self.eat();
    ...
    fn validate_assignment_target(&self, expr: &Expr) -> Result<()> {
        match expr.kind {
            ExprKind::Ident(_) | ExprKind::Index(_, _) | ExprKind::Field { .. } => Ok(()),

            _ => Err(Error::parse("invalid assignment target", expr.span)),
        }
    }
    ...
    fn parse_range(&mut self) -> Result<Expr> {
        ...
        let (inclusive, operator_span, end) = self.parse_range_tail()?;

        let end = end
            .ok_or_else(|| Error::parse("range expression requires an end value", operator_span))?;
    ...
    fn parse_range_tail(&mut self) -> Result<(bool, Span, Option<Expr>)> {
        let operator = self.peek().clone();

        let inclusive = match operator.kind {
            TokenKind::DoubleDot => {
                self.pos += 1;
                false
            },
            ...
            _ => {
                return Err(Error::parse("expected range operator", operator.span));
            },
        };
        ...
        Ok((inclusive, operator.span, end))
    }
    ...
    fn is_range_operator(&self) -> bool {
        matches!(
            self.peek().kind,
            TokenKind::DoubleDot | TokenKind::DoubleDotEq
        )
    }
    ...
    fn parse_control(&mut self) -> Result<Expr> {
        ...
        match self.peek().kind {
            ...
            TokenKind::Break => {
                ...
                Ok(Expr::new(ExprKind::Break, token.span))
            },
    ...
    fn is_drop_statement(&self) -> bool {
        self.at_ident("drop") && matches!(self.peek_n(1).kind, TokenKind::Ident(_))
    }
    ...
    fn parse_return(&mut self) -> Result<Expr> {
        ...
        let span = value
            .as_ref()
            .map(|value| start.join(value.span))
            .unwrap_or(start);
    ...
    fn parse_drop(&mut self) -> Result<Expr> {
        let start = self.eat_ident("drop")?;

        let (name, end) = self.eat_ident_name()?;

        Ok(Expr::new(ExprKind::Drop(name), start.join(end)))
    }
    ...
    fn parse_let(&mut self) -> Result<Expr> {
        self.parse_let_with_visibility(Visibility::Private)
    }

    fn parse_let_with_visibility(&mut self, visibility: Visibility) -> Result<Expr> {
    ...
    fn parse_public_declaration(&mut self) -> Result<Expr> {
        self.expect(TokenKind::Pub)?;

        match self.peek().kind {
            TokenKind::Let => self.parse_let_with_visibility(Visibility::Public),

            TokenKind::Struct => {
                self.parse_struct_or_class_with_visibility(Visibility::Public, false)
            },

            TokenKind::Class => {
                self.parse_struct_or_class_with_visibility(Visibility::Public, true)
            },
    ...
    fn parse_struct_or_class(&mut self, is_class: bool) -> Result<Expr> {
        self.parse_struct_or_class_with_visibility(Visibility::Private, is_class)
    }
    ...
    fn parse_struct_or_class_with_visibility(
        ...
        let start = if is_class {
            self.expect(TokenKind::Class)?.span
        } else {
            self.expect(TokenKind::Struct)?.span
        };
            ...
            if self.eat_if(TokenKind::Equals) {
                ...
                match &value.kind {
                    ExprKind::Lambda(params, _) => {
                        ...
                        methods.push((member_name, Box::new(value)));
                    },

                    _ => {
                        fields.push((member_name, Some(Box::new(value))));
                    },
                }
            } else {
                fields.push((member_name, None));
            }
        ...
        Ok(Expr::new(kind, start.join(close)))
    }
    ...
    fn parse_enum(&mut self) -> Result<Expr> {
        ...
        Ok(Expr::new(
            ExprKind::EnumDecl(EnumDef { name, variants }),
            start.join(end),
        ))
    }
    ...
    fn parse_import(&mut self) -> Result<Expr> {
        ...
        let first_name = match first.kind {
            ...
            _ => return Err(Error::parse("import expects a module name", first.span)),
        };
        ...
        while self.eat_if(TokenKind::Dot) {
            ...
            let name = match token.kind {
                ...
                _ => return Err(Error::parse("expected module name after '.'", token.span)),
            };
        ...
        let alias = if self.eat_if(TokenKind::As) {
            let token = self.peek().clone();
            ...
            let name = match token.kind {
                ...
                _ => return Err(Error::parse("expected identifier after 'as'", token.span)),
            };
        ...
        Ok(Expr::new(
            ExprKind::Import { path: parts, alias },
            start.join(end_span),
        ))
    }
    ...
    fn parse_use(&mut self) -> Result<Expr> {
        ...
        let first_name = match first.kind {
            ...
            _ => {
                return Err(Error::parse("use expects a module name", first.span));
            },
        };
        ...
        while self.eat_if(TokenKind::Dot) {
            ...
            let name = match token.kind {
                ...
                _ => {
                    return Err(Error::parse("expected module name after '.'", token.span));
                },
            };
    ...
    fn parse_if(&mut self) -> Result<Expr> {
        ...
        Ok(Expr::new(
            ExprKind::If(Box::new(cond), Box::new(then_branch), else_branch),
            start.join(end),
        ))
    }
    ...
    fn parse_while(&mut self) -> Result<Expr> {
        ...
        Ok(Expr::new(
            ExprKind::While(Box::new(condition), Box::new(body.clone())),
    ...
    fn parse_or(&mut self) -> Result<Expr> {
        ...
        while self.check(TokenKind::Or) {
            ...
            left = Expr::new(
                ExprKind::Binary(BinOp::Or, Box::new(left), Box::new(right)),
                span,
            );
        }
    ...
    fn parse_and(&mut self) -> Result<Expr> {
        ...
        while self.check(TokenKind::And) {
            ...
            left = Expr::new(
                ExprKind::Binary(BinOp::And, Box::new(left), Box::new(right)),
                span,
            );
        }
    ...
    fn parse_unary(&mut self) -> Result<Expr> {
        match self.peek().kind {
            TokenKind::Minus => {
                ...
                Ok(Expr::new(
                    ExprKind::Neg(Box::new(expr.clone())),
            ...
            TokenKind::Not => {
                ...
                Ok(Expr::new(
                    ExprKind::Not(Box::new(expr.clone())),
                    start.join(expr.span),
                ))
            },
    ...
    fn parse_power(&mut self) -> Result<Expr> {
        ...
        if self.eat_if(TokenKind::DoubleStar) {
            let right = self.parse_unary()?;

            let span = left.span.join(right.span);

            Ok(Expr::new(
                ExprKind::Binary(BinOp::Pow, Box::new(left), Box::new(right)),
                span,
            ))
        } else {
            Ok(left)
        ...
        loop {
            match self.peek().kind.clone() {
                TokenKind::LParen => {
                    ...
                    if !self.is_adjacent(expr.span, next.span) {
                        break;
                    }
                ...
                TokenKind::Question => {
                    ...
                    if !self.is_adjacent(expr.span, token.span) {
                        break;
                    }

                    let start = expr.span;

                    let question = self.eat();

                    expr = Expr::new(ExprKind::Try(Box::new(expr)), start.join(question.span));
                },
    ...
    fn is_match_arm_start(&self) -> bool {
        let opening = match self.peek().kind {
        ...
        while !stack.is_empty() {
            match self.peek_n(offset).kind {
                ...
                TokenKind::RParen | TokenKind::RBracket => {
                    let current = self.peek_n(offset).kind.clone();
        ...
        self.peek_n(offset).kind == TokenKind::FatArrow
    }

    #[inline]
    fn is_adjacent(&self, left: Span, right: Span) -> bool {
    ...
    fn parse_args(&mut self) -> Result<Vec<CallArg>> {
        ...
        loop {
            let is_named = matches!(self.peek().kind, TokenKind::Ident(_))
                && self.peek_n(1).kind == TokenKind::Equals;
    ...
    fn parse_index_component(&mut self) -> Result<IndexExpr> {
        let start = if self.is_range_operator() {
            None
        } else {
            Some(Box::new(self.parse_control()?))
        };

        if self.is_range_operator() {
            let (inclusive, _operator_span, end) = self.parse_range_tail()?;
        ...
        match start {
            Some(expr) => Ok(IndexExpr::Single(expr)),

            None => Err(Error::parse("expected index expression", self.peek().span)),
        }
    }

    fn parse_index_expr(&mut self) -> Result<IndexExpr> {
        let first = self.parse_index_component()?;
    ...
    fn parse_primary(&mut self) -> Result<Expr> {
        ...
        match token.kind {
            ...
            TokenKind::Ident(value) => {
                ...
                Ok(Expr::new(ExprKind::Ident(value), token.span))
            },
    ...
    fn parse_dict(&mut self) -> Result<Expr> {
        ...
        if self.check(TokenKind::RBrace) {
            let close = self.eat().span;

            return Ok(Expr::new(ExprKind::Dict(entries), open.join(close)));
        }
        ...
        Ok(Expr::new(ExprKind::Dict(entries), open.join(close)))
    }
    ...
    fn parse_list(&mut self) -> Result<Expr> {
        ...
        loop {
            let first = self.parse_control()?;

            if self.is_range_operator() {
                let (inclusive, operator_span, end) = self.parse_range_tail()?;

                let end = end.ok_or_else(|| {
                    Error::parse("list range requires an end value", operator_span)
                })?;

                items.push(ListItem::Range(IndexExpr::Range {
    ...
    fn parse_lambda(&mut self) -> Result<Expr> {
        ...
        let body = if self.eat_if(TokenKind::LBrace) {
            let block = self.parse_block_contents()?;
    ...
    fn looks_like_dict(&self) -> bool {
        if self.peek_n(1).kind == TokenKind::RBrace {
            return true;
        }

        matches!(
            (self.peek_n(1).kind.clone(), self.peek_n(2).kind.clone(),),
            (TokenKind::Str(_) | TokenKind::Ident(_), TokenKind::Colon)
        )
    }
    ...
    fn parse_paren_expr(&mut self) -> Result<Expr> {
        ...
        if self.check(TokenKind::RParen) {
            ...
            return Ok(Expr::new(ExprKind::Unit, start.join(end)));
        }
        ...
        if !self.eat_if(TokenKind::Comma) {
            ...
            return Ok(Expr::new(first.kind, start.join(end)));
        }
    ...
    fn parse_pattern(&mut self) -> Result<Pattern> {
        match self.peek().kind.clone() {
            ...
            _ => Err(Error::parse("expected pattern", self.peek().span)),
        }
    }

    fn parse_ident_pattern(&mut self, first: String) -> Result<Pattern> {
        ...
        if path.len() == 1 && !self.check(TokenKind::LParen) {
            return Ok(Pattern::Ident(path.into_iter().next().unwrap()));
        }
    ...
    fn parse_struct_pattern(&mut self, path: Vec<String>) -> Result<Pattern> {
        self.expect(TokenKind::LBrace)?;
        ...
        while !self.check(TokenKind::RBrace) {
            ...
            let pattern = if self.eat_if(TokenKind::Colon) {
                self.parse_pattern()?
            } else {
...
fn assignment_binop(token: &TokenKind) -> Option<BinOp> {
    match token {
