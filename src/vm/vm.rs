...
impl Vm {
    ...
    fn pop(&mut self) -> Result<Value> {
        self.stack
            .pop()
            .ok_or_else(|| Error::new(ErrorKind::Runtime, "VM stack underflow", None))
    }
    ...
    pub fn run(&mut self, chunk: Rc<Chunk>) -> Result<Value> {
        ...
        self.run_with_module_and_path(chunk, module, None)
    }

    pub fn run_with_module(&mut self, chunk: Rc<Chunk>, module: ModuleRef) -> Result<Value> {
        self.run_with_module_and_path(chunk, module, None)
    }
    ...
    pub fn run_repl(&mut self, chunk: Rc<Chunk>) -> Result<Value> {
        ...
        let locals = std::mem::take(&mut self.repl_locals);

        let cells = std::mem::take(&mut self.repl_cells);
        ...
        if cells.len() < local_count {
            cells.resize(local_count, None);
        }
        ...
        match self.execute_until_depth(0) {
            Ok(ExecutionResult { value, frame }) => {
                self.repl_locals = frame.locals;

                self.repl_cells = frame.cells.unwrap_or_default();
    ...
    fn execute(&mut self) -> Result<Value> {
        Ok(self.execute_until_depth(0)?.value)
    }
    ...
    fn execute_module(
        ...
        Ok(self.execute_until_depth(caller_depth)?.value)
    }

    fn execute_until_depth(&mut self, target_depth: usize) -> Result<ExecutionResult> {
        loop {
            ...
            match opcode {
                OpCode::Constant => {
                    let value = self
                        ...
                        .cloned()
                        .ok_or_else(|| {
                            Error::new(ErrorKind::Runtime, "constant index out of bounds", None)
                        })?;
                ...
                OpCode::Dup => {
                    let value = self.stack.last().cloned().ok_or_else(|| {
                        Error::new(ErrorKind::Runtime, "VM stack underflow", None)
                    })?;
                ...
                | OpCode::Geq => {
                    let binop =
                        Self::opcode_to_binop(opcode).expect("numeric opcode must map to BinOp");

                    self.binary_op(binop)?;
                },
                ...
                OpCode::Jump => {
                    self.current_frame_mut().ip = operand as usize;
                },
                ...
                OpCode::LoadLocal => {
                    ...
                    let value = {
                        ...
                        match &frame.cells {
                            Some(cells) => {
                                if let Some(Some(cell)) = cells.get(slot) {
                                    ...
                                    frame.locals.get(slot).cloned().ok_or_else(|| {
                                        Error::new(
                                            ErrorKind::Runtime,
                                            format!("local slot out of bounds: {}", slot),
                                            None,
                                        )
                                    })?
                                }
                            ...
                            None => frame.locals.get(slot).cloned().ok_or_else(|| {
                                Error::new(
                                    ErrorKind::Runtime,
                                    format!("local slot out of bounds: {}", slot),
                                    None,
                ...
                OpCode::StoreLocal => {
                    ...
                    if let Some(cells) = frame.cells.as_mut() {
                        if cells.len() <= slot {
                            cells.resize(slot + 1, None);
                        }

                        if let Some(cell) = cells[slot].clone() {
                ...
                OpCode::LoadBuiltin => {
                    ...
                    let builtin = crate::stdlib::builtin::get(name.as_str()).ok_or_else(|| {
                        Error::new(
                            ErrorKind::Runtime,
                            format!("builtin '{}' is not registered", name),
                            None,
                        )
                    })?;
                ...
                OpCode::LoadUpvalue => {
                    ...
                    let cell = self
                        ...
                        .cloned()
                        .ok_or_else(|| {
                            Error::new(ErrorKind::Runtime, "upvalue slot out of bounds", None)
                        })?;
                ...
                OpCode::Closure => {
                    ...
                    let Value::FunctionProto(function) = value else {
                        return Err(Error::new(
                            ErrorKind::Type,
                ...
                OpCode::NewList => {
                    let list = List::with_capacity(operand as usize);

                    self.push(Value::List(list));
                },

                OpCode::NewDict => {
                    self.push(Value::Dict(Rc::new(RefCell::new(HashMap::with_capacity(
                        operand as usize,
                    )))));
                },

                OpCode::NewClass => {
                    let (field_count, method_count) = decode_class_counts(operand);

                    let required = 1 + field_count * 2 + method_count * 2;
                    ...
                    for _ in 0..method_count {
                        ...
                        let (name, closure) = match (name, closure) {
                            (Value::Str(name), Value::Closure(closure)) => (name, closure),
                        ...
                        method_values.push((name.to_string(), closure));
                    }
                    ...
                    for _ in 0..field_count {
                        ...
                        field_values.push(FieldDefinition::new(name.as_str(), default));
                    }
                    ...
                    let methods: HashMap<String, ClosureRef> = method_values.into_iter().collect();

                    let class = Class::new(class_name.as_str(), field_values, methods);
                ...
                OpCode::ListAppend => {
                    ...
                    let list = self.stack.last().cloned().ok_or_else(|| {
                        Error::new(ErrorKind::Runtime, "list append stack underflow", None)
                    })?;
                ...
                OpCode::ListExtendRange => {
                    ...
                    let list = match self.stack.last().cloned() {
                    ...
                    let inclusive = operand != 0;

                    let end = if inclusive {
                        end.checked_add(1).ok_or_else(|| {
                            Error::new(ErrorKind::Overflow, "range upper bound overflow", None)
                        })?
                    } else {
                        end
                    };
                    ...
                    if start < end {
                        let count = end.checked_sub(start).ok_or_else(|| {
                            Error::new(ErrorKind::Overflow, "range length overflow", None)
                        })?;

                        let mut list = list.as_vec_mut();
                ...
                OpCode::NewRange => {
                    ...
                    self.push(Value::Range(start, end, operand != 0));
                },
                ...
                OpCode::IndexGet => {
                    ...
                    match (object, index) {
                        (Value::List(list), Value::Int(index)) => {
                            ...
                            let value = list.get(index as usize).ok_or_else(|| {
                                Error::new(ErrorKind::Index, "list index out of bounds", None)
                            })?;
                        ...
                        (Value::Tuple(tuple), Value::Int(index)) => {
                            ...
                            let value = tuple.get(index as usize).cloned().ok_or_else(|| {
                                Error::new(ErrorKind::Index, "tuple index out of bounds", None)
                            })?;
                        ...
                        (Value::Series(series), Value::Int(index)) => {
                            ...
                            let value = series.get(index as usize).ok_or_else(|| {
                                Error::new(ErrorKind::Index, "series index out of bounds", None)
                            })?;
                        ...
                        (Value::DataFrame(df), Value::Str(name)) => {
                            let column = df.column(name.as_str()).ok_or_else(|| {
                                Error::new(
                                    ErrorKind::Index,
                                    format!("DataFrame column not found: '{}'", name),
                                    None,
                                )
                            })?;

                            self.push(Value::Series(column));
                        },

                        (Value::DataFrame(df), Value::Int(index)) => {
                            if index < 0 {
                                return Err(Error::new(
                                    ErrorKind::Index,
                            ...
                            let row = df.row(index as usize).ok_or_else(|| {
                                Error::new(
                                    ErrorKind::Index,
                            ...
                            self.push(Value::Dict(row));
                        },

                        (Value::Dict(dict), Value::Str(key)) => {
                            let value =
                                dict.borrow().get(key.as_str()).cloned().ok_or_else(|| {
                                    Error::new(
                                        ErrorKind::Index,
                                        format!("dict key not found: '{}'", key,),
                                        None,
                                    )
                                })?;
                ...
                OpCode::IndexSet => {
                    ...
                    let object_index = self.stack.len() - 3;

                    let index_index = self.stack.len() - 2;

                    let value_index = self.stack.len() - 1;

                    let object = self.stack[object_index].clone();

                    let index = self.stack[index_index].clone();
                    ...
                    match (object, index) {
                        (Value::List(list), Value::Int(index)) => {
                             ...
                             */
                            list.set(index as usize, value.clone())
                ...
                OpCode::FieldGet => {
                    ...
                    let field = match self.stack[self.stack.len() - 1].clone() {
                    ...
                    let object = self.stack[self.stack.len() - 2].clone();

                    let value = self.resolve_field(object, field.as_str())?;
                ...
                OpCode::FieldSet => {
                    ...
                    let Value::Str(field) = field else {
                        return Err(Error::new(ErrorKind::Type, "field name must be Str", None));
                    };
                    ...
                    match object {
                        Value::Object(object) => {
                            ...
                            if class.field(field.as_str()).is_none() {
                                return Err(Error::new(
                                    ErrorKind::Name,
                            ...
                            object.borrow_mut().set_field(field.as_str(), value.clone());
                ...
                OpCode::EnumFieldGet => {
                    ...
                    let Value::EnumValue(value) = object else {
                        return Err(Error::new(
                            ErrorKind::Type,
                    ...
                    let field = value.field(index as usize).ok_or_else(|| {
                        Error::new(ErrorKind::Index, "enum field index out of bounds", None)
                    })?;
                ...
                OpCode::StructFieldGet => {
                    ...
                    let result = value.get_field(field.as_str()).ok_or_else(|| {
                        Error::new(
                            ErrorKind::Name,
                            format!("{} has no field '{}'", value.type_name(), field,),
                            None,
                        )
                    })?;
                ...
                OpCode::RangeInit => {
                    let range_index = operand as usize;
                    ...
                    let range = self
                        ...
                        .copied()
                        .ok_or_else(|| {
                            Error::new(ErrorKind::Runtime, "range loop index out of bounds", None)
                        })?;
                    ...
                    if frame.range_cursors.len() <= range_index {
                        frame.range_cursors.resize(range_index + 1, None);
                    }

                    frame.range_cursors[range_index] = Some(RangeCursor {
                        current: start,
                        end: exclusive_end,
                    });
                },
                ...
                OpCode::RangeNext => {
                    self.advance_range(operand as usize)?;
                },

                OpCode::FusedPipeline => {
                    self.execute_fused_pipeline(operand as usize)?;
                },
                ...
                OpCode::MatchEnum => {
                    ...
                    let Value::Str(enum_name) = enum_name else {
                        return Err(Error::new(
                            ErrorKind::Type,
                    ...
                    let Value::Str(variant) = variant else {
                        return Err(Error::new(
                            ErrorKind::Type,
                    ...
                    let expected_arity = operand as usize;

                    let matched = match &value {
                        Value::EnumValue(enum_value) => {
                            enum_value.enum_name() == enum_name.as_str()
                                && enum_value.variant() == variant.as_str()
                                && enum_value.fields().len() == expected_arity
                        },
                ...
                OpCode::MatchStruct => {
                    ...
                    let matched = match &value {
                        Value::Struct(value) => {
                            ...
                            value.type_name() == name.as_str() && fields.len() >= operand as usize
                        },
                ...
                OpCode::LoadModule => {
                    ...
                    let module = if reference.namespace {
                        self.load_module_namespace(&reference.path)?
                    } else {
                        self.load_module(&reference.path)?
                    };
                ...
                OpCode::Export => {
                    ...
                    let module =
                        self.current_frame().module.clone().ok_or_else(|| {
                            Error::new(ErrorKind::Runtime, "no active module", None)
                        })?;

                    module.borrow_mut().set_exported(name.as_str(), value);
                },

                OpCode::Return => {
                    let result = self.pop()?;

                    if let Some(execution) = self.return_from_current_frame(result, target_depth)? {
                        return Ok(execution);
                    }
                },
                ...
                OpCode::Try => {
                    ...
                    if let Some(execution) = self.try_value(value, target_depth)? {
                        return Ok(execution);
                    }
                },
                ...
                OpCode::Halt => {
                    ...
                    let frame = self.frames.pop().ok_or_else(|| {
                        Error::new(ErrorKind::Runtime, "VM frame underflow", None)
                    })?;
                ...
                _ => {
                    return Err(Error::new(
                        ErrorKind::Runtime,
                        format!("opcode {:?} is not implemented", opcode),
                        None,
                    ));
                },
            }
    ...
    fn execute_fused_pipeline(&mut self, pipeline_index: usize) -> Result<()> {
        let pipeline = self
            ...
            .get(pipeline_index)
            .cloned()
            .ok_or_else(|| Error::new(ErrorKind::Runtime, "pipeline index out of bounds", None))?;

        /*
         * Resolve source exactly once.
         */
        let (start, end, inclusive) = self.resolve_pipeline_source(&pipeline.source)?;

        /*
         * Now the runtime capacity is known.
         */
        let capacity = PipelineProgram::capacity_upper_bound_for_range(
            start,
            end,
            inclusive,
            &pipeline.stages,
        ...
        let list = List::with_capacity(capacity);

        let stage_captures = self.resolve_pipeline_plan_captures(&pipeline.stages)?;

        match &pipeline.plan {
            PipelinePlan::IntRange { stages } => {
    ...
    fn execute_fused_range_pipeline(
        ...
        if stage_captures.len() != stages.len() {
            return Err(Error::new(
                ErrorKind::Runtime,
         ...
         *     Take(n)
         */
        let mut states = stages
            .iter()
            .map(|stage| match stage {
                PipelineStage::Skip { count } => PipelineState::Skip(*count),

                PipelineStage::Take { count } => PipelineState::Take(*count),

                PipelineStage::Map { .. } | PipelineStage::Filter { .. } => PipelineState::None,
            })
            .collect::<Vec<_>>();
        ...
        for current in start..end {
             ...
             */
            let mut value = Value::Int(current);
            ...
            for (index, stage) in stages.iter().enumerate() {
                match stage {
                    ...
                    PipelineStage::Map { expr, .. } => {
                        value = self.eval_pipeline_expr(expr, value, &stage_captures[index])?;
                    },
                    ...
                    PipelineStage::Filter { expr, .. } => {
                        let predicate =
                            self.eval_pipeline_expr(expr, value.clone(), &stage_captures[index])?;

                        let Value::Bool(keep) = predicate else {
                            return Err(Error::new(
                                ErrorKind::Type,
                     ...
                     */
                    PipelineStage::Skip { .. } => {
                        let PipelineState::Skip(remaining) = &mut states[index] else {
                            unreachable!("pipeline state mismatch for Skip");
                        };
                     ...
                     */
                    PipelineStage::Take { .. } => {
                        let PipelineState::Take(remaining) = &mut states[index] else {
                            unreachable!("pipeline state mismatch for Take");
                        };
    ...
    fn execute_int_range_pipeline(
         ...
         */
        let mut skip_remaining = vec![0usize; stages.len()];

        let mut take_remaining: Vec<Option<usize>> = vec![None; stages.len()];

        for (index, stage) in stages.iter().enumerate() {
            match stage {
                IntPipelineStage::Skip(count) => {
                    skip_remaining[index] = *count;
                },

                IntPipelineStage::Take(count) => {
                    take_remaining[index] = Some(*count);
                },

                IntPipelineStage::Map(_) | IntPipelineStage::Filter(_) => {},
            }
        }
            ...
            for (index, stage) in stages.iter().enumerate() {
                match stage {
                    IntPipelineStage::Map(expr) => {
                        value = self.eval_int_pipeline_expr(expr, value, &stage_captures[index])?;
                    },

                    IntPipelineStage::Filter(predicate) => {
                        let keep = self.eval_int_pipeline_predicate(
                            predicate,
                            value,
                    ...
                    IntPipelineStage::Take(_) => {
                        let remaining = take_remaining[index]
                            .as_mut()
                            .expect("missing Take runtime state");
    ...
    fn get_series_property(&self, series: SeriesRef, name: &str) -> Result<Value> {
        match name {
            "name" => Ok(Value::Str(Rc::new(series.name().to_owned()))),

            "len" => Ok(Value::Int(series.len() as i64)),

            "is_empty" => Ok(Value::Bool(series.is_empty())),

            _ => Err(Error::new(
                ErrorKind::Name,
                format!("Series has no property '{}'", name),
                None,
            )),
        }
    }

    fn get_dataframe_property(&self, df: DataFrameRef, name: &str) -> Result<Value> {
        match name {
            "nrows" => Ok(Value::Int(df.nrows() as i64)),

            "ncols" => Ok(Value::Int(df.ncols() as i64)),
    ...
    fn resolve_field(&self, object: Value, field: &str) -> Result<Value> {
        match object {
            Value::Enum(enum_def) => {
                let variant = enum_def.variant(field).ok_or_else(|| {
                    Error::new(
                        ErrorKind::Name,
                        format!("enum '{}' has no variant '{}'", enum_def.name(), field,),
                        None,
                    )
                })?;

                let arity = variant.arity();

                if arity == 0 {
                    Ok(Value::EnumValue(Rc::new(EnumValue::new(
                        enum_def.name(),
                        field,
                        Vec::new(),
                    ))))
                } else {
                    Ok(Value::EnumConstructor(EnumConstructor::new(
                        enum_def, field,
                    )))
                }
            },

            Value::Struct(value) => value.get_field(field).ok_or_else(|| {
                Error::new(
                    ErrorKind::Name,
                    format!("{} has no field '{}'", value.type_name(), field,),
                    None,
            ...
            Value::Object(object) => {
                ...
                object_ref.get_field(field).ok_or_else(|| {
                    Error::new(
                        ErrorKind::Name,
                        format!("{} has no field '{}'", object_ref.type_name(), field,),
                        None,
                    )
                })
            },
            ...
            Value::Module(module) => {
                ...
                module_ref.get_field(field).ok_or_else(|| {
                    Error::new(
                        ErrorKind::Name,
                        format!("{} has no field '{}'", module_ref.name(), field,),
                        None,
                    )
                })
            },
            ...
            Value::DataFrame(df) => self.get_dataframe_property(df, field),
    ...
    fn resolve_pipeline_source(&self, source: &PipelineSource) -> Result<(i64, i64, bool)> {
        match source {
            ...
            PipelineSource::DynamicRange {
                ...
                let captures = self.resolve_pipeline_captures(captures)?;

                let start = self.eval_pipeline_expr(start, Value::Unit, &captures)?;

                let end = self.eval_pipeline_expr(end, Value::Unit, &captures)?;
                ...
                if *require_non_negative_end && end < 0 {
                    return Err(Error::new(
                        ErrorKind::Type,
    ...
    fn resolve_pipeline_captures(&self, specs: &[UpvalueSpec]) -> Result<Vec<CellRef>> {
        let frame = self.current_frame();

        let mut captures = Vec::with_capacity(specs.len());

        for spec in specs {
            match spec {
                UpvalueSpec::Local(slot) => {
                    if let Some(cells) = frame.cells.as_ref() {
                        if let Some(Some(cell)) = cells.get(*slot as usize) {
                    ...
                    let value = frame.locals.get(*slot as usize).cloned().ok_or_else(|| {
                        Error::new(
                            ErrorKind::Runtime,
                            format!("pipeline capture local slot out of bounds: {}", slot),
                            None,
                        )
                    })?;

                    captures.push(Rc::new(RefCell::new(value)));
                },
                ...
                UpvalueSpec::Parent(slot) => {
                    let cell = frame
                        ...
                        .ok_or_else(|| {
                            Error::new(
                                ErrorKind::Runtime,
                                format!("pipeline parent upvalue slot out of bounds: {}", slot),
                                None,
                            )
                        })?;
    ...
    fn resolve_pipeline_plan_captures(
        ...
        for stage in stages {
            match stage {
                PipelineStage::Map { captures, .. } | PipelineStage::Filter { captures, .. } => {
                    result.push(self.resolve_pipeline_captures(captures)?);
                },

                PipelineStage::Skip { .. } | PipelineStage::Take { .. } => {
                    result.push(Vec::new());
                },
            }
        }
    ...
    fn return_from_current_frame(
        ...
        let frame = self
            .frames
            .pop()
            .ok_or_else(|| Error::new(ErrorKind::Runtime, "VM frame underflow", None))?;

        if self.frames.len() == target_depth {
            return Ok(Some(ExecutionResult { value, frame }));
        }
    ...
    fn bind_arguments(
        ...
        let mut bound = vec![Value::Unit; parameters.len()];
            ...
            match name {
                ...
                Some(name) => {
                    let index = parameters
                        .iter()
                        .position(|parameter| parameter.name.as_deref() == Some(name.as_str()))
                        .ok_or_else(|| {
                            Error::new(
                                ErrorKind::Name,
    ...
    fn call_closure_sync(&mut self, closure: ClosureRef, args: Vec<Value>) -> Result<Value> {
        ...
        self.call_closure_sync_named(closure, args, &names)
    }

    fn call_closure_sync1(&mut self, closure: ClosureRef, arg: Value) -> Result<Value> {
        let expected = closure.function.arity as usize;

        if expected != 1 {
            return Err(Error::new(
                ErrorKind::Arity,
                format!("function expects {} arguments, got 1", expected),
                None,
            ));
        }
        ...
        let module = self.current_frame().module.clone();

        let source_path = self.current_frame().source_path.clone();
        ...
        let cells = vec![None; local_count];
    ...
    fn call_closure_sync2(
        ...
        if expected != 2 {
            return Err(Error::new(
                ErrorKind::Arity,
                format!("function expects {} arguments, got 2", expected),
                None,
            ));
        }
    ...
    fn call_closure_sync_named(
        ...
        let bound = self.bind_arguments(&closure.function.parameters, names, args)?;
        ...
        self.frames.push(CallFrame {
            ...
            ip: 0,
            locals,
            cells: vec![None; local_count].into(),
            range_cursors: Vec::new(),
    ...
    fn call_class_value(
        ...
        for field in class.fields() {
            let value = match field.default() {
                Some(closure) => self.call_closure_sync(closure, Vec::new())?,

                None => Value::Unit,
            };

            object.borrow_mut().set_field(field.name(), value);
        }
        ...
        if let Some(constructor) = class.constructor() {
            ...
            call_args.push(Value::Object(object.clone()));
            ...
            self.call_closure_sync_named(constructor, call_args, &call_names)?;
        } else if !args.is_empty() {
            return Err(Error::new(
                ErrorKind::Arity,
                format!("{}() expects 0 arguments, got {}", class.name(), args.len(),),
                None,
            ));
        }
    ...
    fn call_value(
        ...
        match callable {
            Value::Closure(closure) => self.call_closure_sync_named(closure, args, names),

            Value::Builtin(builtin) => {
                ensure_positional_args(names)?;

                builtin(args).map_err(|message| Error::new(ErrorKind::Runtime, message, None))
            },

            Value::Class(class) => self.call_class_value(class, args, names),

            Value::EnumConstructor(constructor) => {
                ensure_positional_args(names)?;

                self.make_enum_value(constructor, args)
            },

            Value::StructType(ty) => {
                ensure_positional_args(names)?;

                self.make_struct_value(ty, args)
            },
    ...
    fn make_enum_value(&mut self, constructor: EnumConstructor, args: Vec<Value>) -> Result<Value> {
    ...
    fn make_struct_value(&mut self, ty: StructTypeRef, args: Vec<Value>) -> Result<Value> {
        let expected = ty.fields().len();
        ...
        let value = StructValue::new(ty, args)
            .map_err(|message| Error::new(ErrorKind::Type, message, None))?;

        Ok(Value::Struct(Rc::new(value)))
    }
    ...
    fn eval_pipeline_expr(
        ...
        match expr {
            PipelineExpr::Input => Ok(input),

            PipelineExpr::Int(value) => Ok(Value::Int(*value)),

            PipelineExpr::Float(value) => Ok(Value::Float(*value)),

            PipelineExpr::Bool(value) => Ok(Value::Bool(*value)),

            PipelineExpr::Str(value) => Ok(Value::Str(Rc::new(value.clone()))),

            PipelineExpr::Capture(index) => {
                let cell = captures.get(*index as usize).cloned().ok_or_else(|| {
                    Error::new(
                        ErrorKind::Runtime,
                        format!("pipeline capture slot out of bounds: {}", index),
                        None,
                    )
                })?;
            ...
            PipelineExpr::Add(left, right) => {
                self.eval_pipeline_binary(BinOp::Add, left, right, input, captures)
            },

            PipelineExpr::Sub(left, right) => {
                self.eval_pipeline_binary(BinOp::Sub, left, right, input, captures)
            },

            PipelineExpr::Mul(left, right) => {
                self.eval_pipeline_binary(BinOp::Mul, left, right, input, captures)
            },

            PipelineExpr::Div(left, right) => {
                self.eval_pipeline_binary(BinOp::Div, left, right, input, captures)
            },

            PipelineExpr::Mod(left, right) => {
                self.eval_pipeline_binary(BinOp::Mod, left, right, input, captures)
            },

            PipelineExpr::Pow(left, right) => {
                self.eval_pipeline_binary(BinOp::Pow, left, right, input, captures)
            },

            PipelineExpr::Eq(left, right) => {
                self.eval_pipeline_binary(BinOp::Eq, left, right, input, captures)
            },

            PipelineExpr::Neq(left, right) => {
                self.eval_pipeline_binary(BinOp::Neq, left, right, input, captures)
            },

            PipelineExpr::Lt(left, right) => {
                self.eval_pipeline_binary(BinOp::Lt, left, right, input, captures)
            },

            PipelineExpr::Leq(left, right) => {
                self.eval_pipeline_binary(BinOp::Leq, left, right, input, captures)
            },

            PipelineExpr::Gt(left, right) => {
                self.eval_pipeline_binary(BinOp::Gt, left, right, input, captures)
            },

            PipelineExpr::Geq(left, right) => {
                self.eval_pipeline_binary(BinOp::Geq, left, right, input, captures)
            },

            PipelineExpr::Neg(expr) => {
                let value = self.eval_pipeline_expr(expr, input, captures)?;
    ...
    fn eval_pipeline_binary(
        ...
        let lhs = self.eval_pipeline_expr(left, input.clone(), captures)?;

        let rhs = self.eval_pipeline_expr(right, input, captures)?;

        apply_binop(op, lhs, rhs).map_err(|message| Error::new(ErrorKind::Runtime, message, None))
    }
    ...
    fn eval_int_pipeline_expr(
        ...
        match expr {
            IntPipelineExpr::Input => Ok(input),

            IntPipelineExpr::Const(value) => Ok(*value),
            ...
            IntPipelineExpr::Add(left, right) => {
                let lhs = self.eval_int_pipeline_expr(left, input, captures)?;

                let rhs = self.eval_int_pipeline_expr(right, input, captures)?;

                lhs.checked_add(rhs).ok_or_else(|| {
                    Error::new(ErrorKind::Overflow, "integer addition overflow", None)
                })
            },

            IntPipelineExpr::Sub(left, right) => {
                let lhs = self.eval_int_pipeline_expr(left, input, captures)?;

                let rhs = self.eval_int_pipeline_expr(right, input, captures)?;

                lhs.checked_sub(rhs).ok_or_else(|| {
                    Error::new(ErrorKind::Overflow, "integer subtraction overflow", None)
                })
            },

            IntPipelineExpr::Mul(left, right) => {
                let lhs = self.eval_int_pipeline_expr(left, input, captures)?;

                let rhs = self.eval_int_pipeline_expr(right, input, captures)?;

                lhs.checked_mul(rhs).ok_or_else(|| {
                    Error::new(ErrorKind::Overflow, "integer multiplication overflow", None)
                })
            },

            IntPipelineExpr::Div(left, right) => {
                let lhs = self.eval_int_pipeline_expr(left, input, captures)?;

                let rhs = self.eval_int_pipeline_expr(right, input, captures)?;

                lhs.checked_div(rhs)
                    .ok_or_else(|| Error::new(ErrorKind::Runtime, "integer division failed", None))
            },

            IntPipelineExpr::Mod(left, right) => {
                let lhs = self.eval_int_pipeline_expr(left, input, captures)?;

                let rhs = self.eval_int_pipeline_expr(right, input, captures)?;

                lhs.checked_rem(rhs)
                    .ok_or_else(|| Error::new(ErrorKind::Runtime, "integer remainder failed", None))
            },

            IntPipelineExpr::Neg(expr) => self
                .eval_int_pipeline_expr(expr, input, captures)?
                .checked_neg()
                .ok_or_else(|| Error::new(ErrorKind::Overflow, "integer negation overflow", None)),
        }
    }
    ...
    fn eval_int_pipeline_predicate(
        ...
        match predicate {
            IntPipelinePredicate::Eq(left, right) => Ok(self
                .eval_int_pipeline_expr(left, input, captures)?
                == self.eval_int_pipeline_expr(right, input, captures)?),

            IntPipelinePredicate::Neq(left, right) => Ok(self
                .eval_int_pipeline_expr(left, input, captures)?
                != self.eval_int_pipeline_expr(right, input, captures)?),

            IntPipelinePredicate::Lt(left, right) => Ok(self
                .eval_int_pipeline_expr(left, input, captures)?
                < self.eval_int_pipeline_expr(right, input, captures)?),

            IntPipelinePredicate::Leq(left, right) => Ok(self
                .eval_int_pipeline_expr(left, input, captures)?
                <= self.eval_int_pipeline_expr(right, input, captures)?),

            IntPipelinePredicate::Gt(left, right) => Ok(self
                .eval_int_pipeline_expr(left, input, captures)?
                > self.eval_int_pipeline_expr(right, input, captures)?),

            IntPipelinePredicate::Geq(left, right) => Ok(self
                .eval_int_pipeline_expr(left, input, captures)?
                >= self.eval_int_pipeline_expr(right, input, captures)?),
        }
    }
    ...
    fn try_value(&mut self, value: Value, target_depth: usize) -> Result<Option<ExecutionResult>> {
        let Value::EnumValue(enum_value) = value.clone() else {
            return Err(Error::new(
                ErrorKind::Type,
                format!(
        ...
        match (enum_value.enum_name(), enum_value.variant()) {
            ("Option", "Some") => {
                ...
                self.push(enum_value.field(0).expect("checked above"));
            ...
            ("Option", "None") => {
                ...
                self.return_from_current_frame(value, target_depth)
            },
    ...
    #[inline(always)]
    fn advance_range(&mut self, range_index: usize) -> Result<()> {
        let range = self
            .current_frame()
            .closure
            .function
        ...
        let cursor = frame
            ...
            .ok_or_else(|| {
                Error::new(ErrorKind::Runtime, "range cursor is not initialized", None)
            })?;

        if cursor.current >= cursor.end {
            frame.ip = range.exit_ip as usize;
        ...
        cursor.current = current
            .checked_add(1)
            .ok_or_else(|| Error::new(ErrorKind::Overflow, "range increment overflow", None))?;

        let value = Value::Int(current);

        let value_slot = range.value_slot as usize;

        /*
         * Local storage invariant.
         */
        if frame.locals.len() <= value_slot {
            frame.locals.resize(value_slot + 1, Value::Unit);
        }

        frame.locals[value_slot] = value.clone();
        ...
        if let Some(cells) = frame.cells.as_mut() {
            if cells.len() <= value_slot {
                cells.resize(value_slot + 1, None);
            }

            if let Some(cell) = cells[value_slot].as_ref() {
                *cell.borrow_mut() = value;
            }
        }
    ...
    #[inline]
    fn opcode_to_binop(opcode: OpCode) -> Option<BinOp> {
        match opcode {
            ...
            OpCode::Pow => Some(BinOp::Pow),

            OpCode::Eq => Some(BinOp::Eq),

            OpCode::Neq => Some(BinOp::Neq),

            OpCode::Lt => Some(BinOp::Lt),

            OpCode::Leq => Some(BinOp::Leq),

            OpCode::Gt => Some(BinOp::Gt),

            OpCode::Geq => Some(BinOp::Geq),

            OpCode::MatMul => Some(BinOp::MatMul),

            _ => None,
        }
    }

    fn binary_op(&mut self, op: BinOp) -> Result<()> {
        let right = self.pop()?;
        let left = self.pop()?;
    ...
    fn capture_local(&mut self, slot: usize) -> Result<CellRef> {
        let frame = self.current_frame_mut();
        ...
        if frame.cells.is_none() {
            ...
            frame.cells = Some(vec![None; local_count]);
        }

        let cells = frame.cells.as_mut().expect("cells must be initialized");
        ...
        let cell = Rc::new(RefCell::new(frame.locals[slot].clone()));

        cells[slot] = Some(cell.clone());

        Ok(cell)
    }

    fn create_closure(&mut self, function: FunctionRef) -> Result<ClosureRef> {
        let mut upvalues = Vec::with_capacity(function.upvalue_specs.len());

        for spec in &function.upvalue_specs {
            let cell = match *spec {
                UpvalueSpec::Local(slot) => self.capture_local(slot as usize)?,

                UpvalueSpec::Parent(index) => self
                    .current_frame()
                    .closure
                    .upvalues
                    .get(index as usize)
                    .cloned()
                    .ok_or_else(|| {
                        Error::new(ErrorKind::Runtime, "parent upvalue out of bounds", None)
                    })?,
            };
        ...
        Ok(Rc::new(Closure { function, upvalues }))
    }

    fn load_module(&mut self, path: &ModulePath) -> Result<ModuleRef> {
        let importing_file = self.current_frame().source_path.clone();
        ...
        if let Some(canonical) = self
            .module_loader
            .try_resolve(path, importing_file.as_deref())?
        {
            return self.load_file_module(canonical, path);
        }
        ...
        if path.parts().len() == 1 {
            ...
            if let Some(module) = self.stdlib_modules.get(name) {
                return Ok(module.clone());
            }

            if let Some(module) = crate::stdlib::load_module(name) {
                self.stdlib_modules.insert(name.clone(), module.clone());
    ...
    fn load_module_namespace(&mut self, path: &ModulePath) -> Result<ModuleRef> {
         ...
         * actual c ModuleRef
         */
        let leaf = self.load_module(path)?;
        ...
        for depth in 0..parts.len() - 1 {
            let prefix = ModulePath::new(parts[..=depth].to_vec());

            if !self.module_namespaces.contains_key(&prefix) {
                let module = Rc::new(RefCell::new(Module::new(prefix.name())));

                self.module_namespaces.insert(prefix, module);
            }
        }
         ...
         * b exports c
         */
        for depth in 0..parts.len() - 2 {
            let parent_path = ModulePath::new(parts[..=depth].to_vec());

            let child_path = ModulePath::new(parts[..=depth + 1].to_vec());
            ...
            let child = self
                ...
                .ok_or_else(|| {
                    Error::new(
                        ErrorKind::Import,
                        format!("internal error: namespace '{}' was not created", child_path),
                        None,
                    )
                })?;
            ...
            parent
                .borrow_mut()
                .set_exported(parts[depth + 1].clone(), Value::Module(child));
        }
         ...
         */
        let parent_depth = parts.len() - 2;

        let parent_path = ModulePath::new(parts[..=parent_depth].to_vec());
        ...
        parent
            .borrow_mut()
            .set_exported(parts[parts.len() - 1].clone(), Value::Module(leaf));
         ...
         * a -> Module("a")
         */
        let root_path = ModulePath::new(vec![parts[0].clone()]);
    ...
    fn load_file_module(
        ...
        let module = Rc::new(RefCell::new(Module::new(module_path.name())));

        self.modules.insert(canonical.clone(), module.clone());

        self.loading_modules.push(canonical.clone());

        let chunk = match self.module_loader.load_chunk(&canonical) {
            Ok(chunk) => chunk,
        ...
        let result = self.execute_module(chunk, module.clone(), canonical.clone());
    ...
    fn iterator_next(&mut self, iterator: IteratorRef) -> Result<IterResult> {
        let kind = {
            ...
            match &*state {
                ...
                IteratorObj::Vector { .. } => 2,

                IteratorObj::Range { .. } => 3,

                IteratorObj::Map { .. } => 4,

                IteratorObj::Filter { .. } => 5,

                IteratorObj::Enumerate { .. } => 6,
                ...
                IteratorObj::Series { .. } => 10,

                IteratorObj::DataFrame { .. } => 11,
            }
        };

        match kind {
            0 | 1 | 2 | 3 => self.iterator_next_base(&iterator),

            4 => {
                let (source, function) = {
                    let state = iterator.borrow();

                    let IteratorObj::Map { source, function } = &*state else {
                        unreachable!();
                    };
                ...
                match self.iterator_next(source)? {
                    IterResult::End => Ok(IterResult::End),

                    IterResult::Item(value) => {
                        let result = self.call_closure_sync1(function, value)?;

                        Ok(IterResult::Item(result))
                    },
                }
            },
            ...
            5 => {
                let (source, predicate) = {
                    ...
                    let IteratorObj::Filter { source, predicate } = &*state else {
                        unreachable!();
                    };

                    (source.clone(), predicate.clone())
                };

                loop {
                    match self.iterator_next(source.clone())? {
                        IterResult::End => return Ok(IterResult::End),

                        IterResult::Item(value) => {
                            let result =
                                self.call_closure_sync1(predicate.clone(), value.clone())?;

                            match result {
                                Value::Bool(true) => return Ok(IterResult::Item(value)),
            ...
            6 => {
                let source = {
                    ...
                    let IteratorObj::Enumerate { source, .. } = &*state else {
                        unreachable!();
                    };
                ...
                match self.iterator_next(source)? {
                    ...
                    IterResult::Item(value) => {
                        let current = {
                            ...
                            let IteratorObj::Enumerate { index, .. } = &mut *state else {
                                unreachable!();
                            };
                        ...
                        Ok(IterResult::Item(Value::Tuple(Rc::new(vec![
            ...
            7 => {
                let (left, right) = {
                    ...
                    let IteratorObj::Zip { left, right } = &*state else {
                        unreachable!();
                    };
                ...
                let left = self.iterator_next(left)?;

                let right = self.iterator_next(right)?;

                match (left, right) {
                    (IterResult::Item(left), IterResult::Item(right)) => {
                        Ok(IterResult::Item(Value::Tuple(Rc::new(vec![left, right]))))
                    },
            ...
            8 => {
                let source = {
                    ...
                    let IteratorObj::Take { source, remaining } = &*state else {
                        unreachable!();
                    };
                ...
                match self.iterator_next(source)? {
                    ...
                    IterResult::Item(value) => {
                        ...
                        let IteratorObj::Take { remaining, .. } = &mut *state else {
                            unreachable!();
                        };

                        *remaining -= 1;

                        Ok(IterResult::Item(value))
                    },
                }
            },
            ...
            9 => {
                ...
                loop {
                    let remaining = {
                        ...
                        let IteratorObj::Skip { remaining, .. } = &*state else {
                            unreachable!();
                        };
                    ...
                    match self.iterator_next(source.clone())? {
                        ...
                        IterResult::Item(_) => {
                            ...
                            let IteratorObj::Skip { remaining, .. } = &mut *state else {
                                unreachable!();
                            };
            ...
            10 | 11 => self.iterator_next_base(&iterator),

            _ => unreachable!(),
        }
    }

    fn iterator_next_base(&mut self, iterator: &IteratorRef) -> Result<IterResult> {
        let kind = {
            let state = iterator.borrow();
            ...
            match &*state {
                ...
                IteratorObj::Series { .. } => 3,

                IteratorObj::DataFrame { .. } => 4,
        ...
        match kind {
            ...
            0 => {
                ...
                let IteratorObj::List { data, index } = &mut *state else {
                    unreachable!();
                };
            ...
            1 => {
                ...
                let IteratorObj::Str { data, byte_index } = &mut *state else {
                    unreachable!();
                };

                let slice = &data[*byte_index..];
                ...
                *byte_index += ch.len_utf8();
                ...
                Ok(IterResult::Item(Value::Str(Rc::new(ch.to_string()))))
            },
            ...
            2 => {
                ...
                let IteratorObj::Vector { data, index } = &mut *state else {
                    unreachable!();
                };
                ...
                let x = match data.borrow().get(*index) {
                    Some(value) => {
                        *index += 1;
                        ...
                        Ok(IterResult::Item(Value::Float(value)))
                    },
            ...
            3 => {
                ...
                let IteratorObj::Series { data, index } = &mut *state else {
                    unreachable!();
                };
            ...
            4 => {
                ...
                let IteratorObj::DataFrame { data, index } = &mut *state else {
                    unreachable!();
                };
                ...
                match data.row(*index) {
                    Some(row) => {
                        *index += 1;
                        ...
                        Ok(IterResult::Item(Value::Dict(row)))
                    },
            ...
            5 => {
                ...
                let IteratorObj::Range { current, end } = &mut *state else {
                    unreachable!();
                };
                ...
                *current += 1;
                ...
                Ok(IterResult::Item(Value::Int(value)))
            },
    ...
    fn collect_iterator(&mut self, iterator: IteratorRef) -> Result<Value> {
        ...
        loop {
            match self.iterator_next(iterator.clone())? {
                IterResult::Item(value) => {
                    list.push(value);
                },
    ...
    fn reduce_iterator(&mut self, iterator: IteratorRef, function: ClosureRef) -> Result<Value> {
        let first = match self.iterator_next(iterator.clone())? {
            IterResult::Item(value) => value,

            IterResult::End => return Ok(Value::Unit),
        };
        ...
        loop {
            match self.iterator_next(iterator.clone())? {
                IterResult::Item(value) => {
                    accumulator = self.call_closure_sync2(function.clone(), accumulator, value)?;
                },

                IterResult::End => return Ok(accumulator),
            }
        }
    }
    ...
    fn any_iterator(&mut self, iterator: IteratorRef, predicate: ClosureRef) -> Result<Value> {
        loop {
            match self.iterator_next(iterator.clone())? {
                IterResult::Item(value) => {
                    let result = self.call_closure_sync1(predicate.clone(), value)?;
                ...
                IterResult::End => return Ok(Value::Bool(false)),
            }
        }
    }

    fn all_iterator(&mut self, iterator: IteratorRef, predicate: ClosureRef) -> Result<Value> {
        loop {
            match self.iterator_next(iterator.clone())? {
                IterResult::Item(value) => {
                    let result = self.call_closure_sync1(predicate.clone(), value)?;
                ...
                IterResult::End => return Ok(Value::Bool(true)),
            }
        }
    }
    ...
    fn numeric_reduce(&mut self, iterator: IteratorRef, op: BinOp) -> Result<Value> {
        let first = match self.iterator_next(iterator.clone())? {
            ...
            IterResult::End => return Ok(Value::Int(if op == BinOp::Mul { 1 } else { 0 })),
        };
        ...
        loop {
            match self.iterator_next(iterator.clone())? {
                IterResult::Item(value) => {
                    accumulator = apply_binop(op, accumulator, value)
                        .map_err(|message| Error::new(ErrorKind::Type, message, None))?;
                },
    ...
    fn extreme_iterator(&mut self, iterator: IteratorRef, maximum: bool) -> Result<Value> {
        ...
        loop {
            match self.iterator_next(iterator.clone())? {
                IterResult::Item(value) => {
                    let op = if maximum { BinOp::Gt } else { BinOp::Lt };

                    let greater = apply_binop(op, value.clone(), extreme.clone())
                        .map_err(|message| Error::new(ErrorKind::Type, message, None))?;

                    let Value::Bool(replace) = greater else {
                        return Err(Error::new(
                            ErrorKind::Type,
                ...
                IterResult::End => return Ok(extreme),
            }
        }
    }

    fn make_iterator(&self, value: Value) -> Result<IteratorRef> {
        IteratorObj::from_value(value).map_err(|message| Error::new(ErrorKind::Type, message, None))
    }
    ...
    fn invoke_module_member(
        ...
        match value {
            Value::Builtin(function) => {
                ...
                function(args).map_err(|message| Error::new(ErrorKind::Runtime, message, None))
            },
    ...
    fn invoke_object_method(
        ...
        let method = class.method(name).ok_or_else(|| {
            Error::new(
                ErrorKind::Name,
        ...
        call_args.push(Value::Object(object));
    ...
    fn invoke_enum_constructor(
        ...
        let variant = enum_def.variant(name).ok_or_else(|| {
            Error::new(
                ErrorKind::Name,
                format!("enum '{}' has no variant '{}'", enum_def.name(), name,),
                None,
            )
        })?;
    ...
    fn current_frame(&self) -> &CallFrame {
        self.frames.last().expect("VM has no current frame")
    }
...
#[inline]
fn ensure_positional_args(names: &[Option<String>]) -> Result<()> {
