...
pub enum IntPipelinePredicate {
    Eq(Box<IntPipelineExpr>, Box<IntPipelineExpr>),

    Neq(Box<IntPipelineExpr>, Box<IntPipelineExpr>),

    Lt(Box<IntPipelineExpr>, Box<IntPipelineExpr>),

    Leq(Box<IntPipelineExpr>, Box<IntPipelineExpr>),

    Gt(Box<IntPipelineExpr>, Box<IntPipelineExpr>),

    Geq(Box<IntPipelineExpr>, Box<IntPipelineExpr>),
}
...
pub enum PipelinePlan {
    ...
    IntRange { stages: Vec<IntPipelineStage> },
}
...
impl PipelineProgram {
    pub fn capacity_upper_bound(&self) -> Result<usize> {
        match self.source {
            PipelineSource::Range {
                start,
                end,
                inclusive,
            } => Self::capacity_upper_bound_for_range(start, end, inclusive, &self.stages),
    ...
    pub fn capacity_upper_bound_for_range(
        ...
        let count = normalized_end
            .checked_sub(start)
            .ok_or_else(|| Error::new(ErrorKind::Overflow, "pipeline range size overflow", None))?;

        let mut capacity = usize::try_from(count)
            .map_err(|_| Error::new(ErrorKind::Overflow, "pipeline range is too large", None))?;

        for stage in stages {
            if let PipelineStage::Take { count } = stage {
                capacity = capacity.min(*count);
            }
        }
