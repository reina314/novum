...
impl ModuleLoader {
    ...
    pub fn resolve(
        ...
        self.try_resolve(module_path, importing_file)?
            .ok_or_else(|| {
                Error::new(
                    ErrorKind::Import,
    ...
    pub fn load_chunk(&mut self, path: &Path) -> Result<Rc<Chunk>> {
        let path = fs::canonicalize(path).map_err(|error| {
            Error::new(
                ErrorKind::Import,
                format!("failed to resolve module '{}': {}", path.display(), error,),
                None,
            )
        })?;

        if let Some(chunk) = self.chunk_cache.get(&path) {
        ...
        let source = fs::read_to_string(&path).map_err(|error| {
            Error::new(
                ErrorKind::Import,
                format!("failed to read module '{}': {}", path.display(), error,),
                None,
            )
        })?;
        ...
        self.chunk_cache.insert(path, chunk.clone());
