...
impl List {
    ...
    pub fn set(&self, index: usize, value: Value) -> Result<(), String> {
        ...
        let slot = elements
            .get_mut(index)
            .ok_or_else(|| format!("list index out of bounds: {}", index))?;
    ...
    #[inline]
    pub fn append(&self, mut values: Vec<Value>) {
        self.elements.borrow_mut().append(&mut values);
    }

    #[inline]
    pub fn extend(&self, values: impl IntoIterator<Item = Value>) {
        self.elements.borrow_mut().extend(values);
    }
    ...
    #[inline]
    pub fn as_vec(&self) -> Ref<'_, Vec<Value>> {
        self.elements.borrow()
    }

    #[inline]
    pub fn as_vec_mut(&self) -> RefMut<'_, Vec<Value>> {
        self.elements.borrow_mut()
    }

    pub fn repeat(&self, count: usize) -> Result<Self, String> {
        if count == 0 {
        ...
        let capacity = elements
            .len()
            .checked_mul(count)
            .ok_or_else(|| "list repetition size overflow".to_string())?;
...
impl Value {
    ...
    pub fn receiver_kind(&self) -> ReceiverKind {
        match self {
            ...
            Self::Range(..) => ReceiverKind::Range,

            Self::Path(_) => ReceiverKind::Path,
    ...
    pub fn eq_values(a: &Self, b: &Self) -> Result<bool, String> {
        Ok(match (a, b) {
            ...
            (Self::Path(a), Self::Path(b)) => a.as_path() == b.as_path(),
    ...
    fn eq_slices(a: &[Value], b: &[Value]) -> Result<bool, String> {
        if a.len() != b.len() {
            return Ok(false);
        }
...
impl fmt::Debug for Value {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Set(set) => write!(f, "{:?}", set.borrow().values()),
...
impl fmt::Display for Value {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Matrix(matrix) => matrix.borrow().fmt_display(f),

            Self::Series(series) => series.fmt_display(f),

            Self::DataFrame(df) => df.fmt_display(f),
...
impl PartialEq for Value {
    fn eq(&self, other: &Self) -> bool {
        Self::eq_values(self, other).unwrap_or(false)
    }
}
