...
pub fn get(name: &str) -> Option<Value> {
    BUILTINS
        ...
        .map(|(_, function)| Value::Builtin(*function))
}
...
fn expect_args(args: &[Value], expected: usize, name: &str) -> Result<(), String> {
...
fn expect_string(value: &Value, name: &str) -> Result<Rc<String>, String> {
    match value {
        Value::Str(s) => Ok(Rc::clone(s)),

        other => Err(format!("{name}() expected Str, got {}", other.type_name())),
    }
}
...
pub fn input(args: Vec<Value>) -> Result<Value, String> {
    if args.len() > 1 {
        return Err("input() expects zero or one argument".into());
    }
...
pub fn str(args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("str() expects exactly 1 argument".into());
    }

    let value = args.into_iter().next().unwrap();

    let text = match value {
        Value::Path(path) => path.to_string_lossy().to_owned(),
...
pub fn int(args: Vec<Value>) -> Result<Value, String> {
    ...
    match args.into_iter().next().unwrap() {
        ...
        Value::Float(value) => {
            if !value.is_finite() {
                return Err("cannot convert non-finite Float to Int".into());
            }
...
pub fn bool(args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("bool() expects exactly 1 argument".into());
    }

    match args.into_iter().next().unwrap() {
        Value::Bool(value) => Ok(Value::Bool(value)),

        Value::Str(text) => match text.trim() {
...
pub fn path(mut args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("path() expects exactly 1 argument".into());
    }

    let path = match args.remove(0) {
        Value::Str(value) => PathValue::new(value.as_ref()),

        Value::Path(value) => value.as_ref().clone(),
...
pub fn iter(mut args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("iter() takes exactly 1 argument".into());
    }
...
pub fn series(mut args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 2 {
        return Err("series() expects exactly 2 arguments".into());
    }
    ...
    Ok(Value::Series(Rc::new(Series::new(name, data))))
}

pub fn dataframe(mut args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("dataframe() expects exactly 1 argument".into());
    }

    let columns = match args.pop().unwrap() {
        Value::List(list) => {
            let mut columns = Vec::with_capacity(list.len());

            for value in list.iter_cloned() {
    ...
    let dataframe = DataFrame::from_series(columns)?;
...
pub fn zip(mut args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 2 {
        return Err("zip() expects exactly 2 arguments".into());
    }

    let left = IteratorObj::from_value(args.remove(0))?;

    let right = IteratorObj::from_value(args.remove(0))?;
...
pub fn enumerate(mut args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("enumerate() expects exactly 1 argument".into());
    }

    let source = IteratorObj::from_value(args.remove(0))?;
...
pub fn zeros(mut args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("zeros() expects exactly 1 argument".into());
    }

    let count = match args.remove(0) {
        Value::Int(value) if value >= 0 => value as usize,

        Value::Int(_) => return Err("zeros() does not accept a negative count".into()),

        other => return Err(format!("zeros() expects Int, got {}", other.type_name())),
    };

    Ok(Value::List(List::new(vec![Value::Int(0); count])))
}
    ...
    match args.len() {
        1 => {
            let end = match &args[0] {
                ...
                other => {
                    return Err(format!("range() expected Int, got {}", other.type_name()));
                },
            };

            if end < 0 {
                return Err("range() requires a non-negative argument".into());
            }

            let values = (0..end).map(Value::Int).collect::<Vec<_>>();
        ...
        2 => {
            ...
            let values = (start..end).map(Value::Int).collect::<Vec<_>>();

            Ok(Value::List(List::new(values)))
        },

        _ => Err("range() expects 1 or 2 arguments".into()),
    }
}
...
pub fn len(args: Vec<Value>) -> Result<Value, String> {
    ...
    let length = match &args[0] {
        ...
        other => {
            return Err(format!("len() does not support {}", other.type_name()));
        },
    };
...
pub fn random(args: Vec<Value>) -> Result<Value, String> {
    if !args.is_empty() {
        return Err("random() expects no arguments".into());
    }
...
pub fn randint(args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 2 {
        return Err("randint() expects exactly 2 arguments".into());
    }
    ...
    let min = match &args[0] {
        ...
        other => {
            return Err(format!("randint() expected Int, got {}", other.type_name()));
        },
    };
    ...
    if min > max {
        return Err("randint() requires min <= max".into());
    }

    Ok(Value::Int(rand::random_range(min..=max)))
}
...
pub fn is_type(args: Vec<Value>) -> Result<Value, String> {
    ...
    Ok(Value::Bool(args[0].type_name() == type_name.as_str()))
}

pub fn sleep(args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err("sleep() expects exactly 1 argument".into());
    }

    let ms = match &args[0] {
        Value::Int(ms) if *ms >= 0 => *ms,

        Value::Int(_) => return Err("sleep() requires a non-negative value".into()),

        other => return Err(format!("sleep() expected Int, got {}", other.type_name())),
    };
...
pub fn assert(args: Vec<Value>) -> Result<Value, String> {
    ...
    match &args[0] {
        ...
        other => Err(format!("assert() expected Bool, got {}", other.type_name())),
    }
}

pub fn panic(args: Vec<Value>) -> Result<Value, String> {
    if args.len() > 1 {
        return Err("panic() expects zero or one argument".into());
    }

    let message = match args.first() {
        None => "panic".to_string(),

        Some(Value::Str(message)) => message.as_ref().clone(),
