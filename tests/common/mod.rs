...
pub fn unwrap_value(source: &str) -> Value {
    match run(source) {
        Ok(value) => value,
...
pub fn assert_int(source: &str, expected: i64) {
    match unwrap_value(source) {
        ...
        actual => {
            panic!("expected Int({expected}), got {actual:?}\nsource:\n{source}");
        },
    }
}
...
pub fn assert_float(source: &str, expected: f64) {
    match unwrap_value(source) {
        ...
        actual => {
            panic!("expected Float({expected}), got {actual:?}\nsource:\n{source}");
        },
    }
}
...
pub fn assert_bool(source: &str, expected: bool) {
    match unwrap_value(source) {
        ...
        actual => {
            panic!("expected Bool({expected}), got {actual:?}\nsource:\n{source}");
        },
    }
}

pub fn assert_string(source: &str, expected: &str) {
    match unwrap_value(source) {
        Value::Str(actual) => {
        ...
        actual => {
            panic!("expected Str({expected:?}), got {actual:?}\nsource:\n{source}");
        },
    }
}

pub fn assert_list(source: &str, expected: &[i64]) {
    match unwrap_value(source) {
        Value::List(list) => {
            assert_eq!(list.len(), expected.len(), "\nsource:\n{source}");

            for (index, expected_value) in expected.iter().enumerate() {
                let actual = list.get(index).unwrap_or_else(|| {
                    panic!("missing list element at index {index}\nsource:\n{source}")
                });
                ...
                match actual {
                    ...
                    other => {
                        panic!("expected Int at index {index}, got {other:?}\nsource:\n{source}");
                    },
                }
            }
        },
...
pub fn assert_float_list(source: &str, expected: &[f64]) {
    match unwrap_value(source) {
        Value::List(list) => {
            assert_eq!(list.len(), expected.len(), "\nsource:\n{source}");
...
pub fn assert_matrix(source: &str, expected: &[&[f64]]) {
    match unwrap_value(source) {
        Value::Matrix(matrix) => {
            ...
            let expected_cols = expected.first().map(|row| row.len()).unwrap_or(0);
            ...
            for row in 0..expected.len() {
                ...
                for col in 0..expected_cols {
                    let actual = matrix.get(row, col).unwrap_or_else(|| {
                        panic!("missing matrix element ({row}, {col})\nsource:\n{source}")
                    });
        ...
        actual => {
            panic!("expected Matrix, got {actual:?}\nsource:\n{source}");
        },
    }
}

pub fn assert_vector(source: &str, expected: &[f64]) {
    match unwrap_value(source) {
        Value::Vector(vector) => {
            ...
            for (index, expected_value) in expected.iter().enumerate() {
                let actual = vector.get(index).unwrap_or_else(|| {
                    panic!("missing vector element at index {index}\nsource:\n{source}")
                });
        ...
        actual => {
            panic!("expected Vector, got {actual:?}\nsource:\n{source}");
        },
    }
}

pub fn assert_error_kind(source: &str, expected: ErrorKind) {
    match run(source) {
        Ok(value) => {
            panic!("expected {expected:?} error, got {value:?}\nsource:\n{source}");
        },
